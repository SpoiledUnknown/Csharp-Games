...
namespace ChessLogic
{
    public class Board
    {
        ...
        public IEnumerable<Position> PiecePositons()
        {
            for (int r = 0; r < 8; r++)
            {
                for (int c = 0; c < 8; c++)
        ...
        {
            return PiecePositons().Where(position => this[position].Color == player);
        }
        ...
        public Board Copy()
        {
            ...
            foreach (Position pos in PiecePositons())
            {
        ...
        private Position FindPiece(Player player, PieceType type)
        {
            return PiecePositionFor(player).First(pos => this[pos].Type == type);
        }
        ...
        private bool HasPawnInPosition(Player player, Position[] pawnPosition, Position skiPos)
        {
            foreach (var pos in pawnPosition.Where(IsInside))
            {
                Piece piece = this[pos];
